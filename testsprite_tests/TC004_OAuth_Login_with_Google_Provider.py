...
async def run_test():
    ...
    try:
        ...
        # Click on 'Вход' (Login) link to navigate to login page.
        elem = frame.locator('xpath=html/body/div/div/header/div/div/div[2]/a[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click on the 'Войти через Google' button to start Google OAuth authentication.
        frame = context.pages[-1]
        # Click on 'Войти через Google' button to initiate Google OAuth login.
        elem = frame.locator('xpath=html/body/div/div/main/div/div/div[2]/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input the email 'demo@mooncode.io' into the email field and click Next.
        frame = context.pages[-1]
        ...
        try:
            ...
            raise AssertionError("Test case failed: User login via Google OAuth did not complete successfully. The user was not authenticated or redirected to the dashboard as expected.")
