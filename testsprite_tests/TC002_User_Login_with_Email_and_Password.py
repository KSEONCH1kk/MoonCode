...
async def run_test():
    ...
    try:
        ...
        # Click on 'Вход' (Login) link to go to login page
        ...
        frame = context.pages[-1]
        # Click the 'Войти' (Login) button to submit the login form
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Дашборд').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Мои курсы').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Мои испытания').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Мои решения').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Рейтинг').first).to_be_visible(timeout=30000)
